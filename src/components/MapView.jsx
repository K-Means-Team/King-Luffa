...
export function MapView({ userPosition, otherPlayers, hillPolygon }) {
  ...
  const hillData = useMemo(() => hillToFeatureCollection(hillPolygon), [hillPolygon]);
  ...
  const onLoad = useCallback(() => {
    ...
    if (map && userPosition?.lng != null && userPosition?.lat != null) {
      map.jumpTo({ center: [userPosition.lng, userPosition.lat], zoom: 15 });
    }
  }, [userPosition?.lng, userPosition?.lat]);

  useEffect(() => {
    if (userPosition?.lng == null || userPosition?.lat == null) return;
    const map = mapRef.current?.getMap?.();
    if (!map) return;
    const now = Date.now();
  ...
  if (!token) {
    return (
      <div className="map-fallback">
        Set <code>VITE_MAPBOX_ACCESS_TOKEN</code> in <code>.env</code> to load the map.
      </div>
    );
  }
  ...
  return (
    ...
    >
      {hillData.features.length > 0 && (
        <Source id="hill" type="geojson" data={hillData}>
          <Layer
            id="hill-fill"
            type="fill"
            paint={{
      ...
      {userPosition?.lng != null && userPosition?.lat != null && (
