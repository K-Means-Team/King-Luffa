...
function GameScreen() {
	...
	const hillPolygon = useMemo(() => {
		...
		return hillPolygonFromCenter(
			demoHill.lng,
			demoHill.lat,
			demoHill.radiusMeters,
		);
	}, [demoHill]);
	...
	useEffect(() => {
		...
		const detach = attachGameHandlers(socket, {
			...
			onHill: (p) => {
				...
				const radiusMeters =
					p.radiusMeters ?? p.radius ?? 80;
				if (lng != null && lat != null)
					setHillMeta({ lng, lat, radiusMeters });
			},
			onPoints: (p) =>
				setPoints(Number(p.value ?? p.points ?? 0)),
